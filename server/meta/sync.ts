...
export async function runInstagramSync(options: {
  db: BetterSQLite3Database;
  config: AppConfig;
  pageId: string;
  igId: string;
}): Promise<void> {
  const { db, config, pageId, igId } = options;
  const pageRow = db
    .select()
    .from(metaPages)
    .where(eq(metaPages.id, pageId))
  ...
  const igAsset = db.select().from(igAssets).where(eq(igAssets.id, igId)).get();
  if (!igAsset) {
    throw new Error('Instagram asset not found. Load IG assets first.');
  }
  ...
  const syncState = db
    ...
    .from(syncStates)
    .where(
      and(eq(syncStates.pageId, pageId), eq(syncStates.platform, 'instagram')),
    )
    .orderBy(desc(syncStates.updatedAt))
    .get();
  ...
  const conversationsList = await fetchIgConversations({
    pageId,
    accessToken: pageToken,
    version: config.metaApiVersion,
    since: sinceUnix,
  ...
  const processConversation = async (convo: MetaConversation) => {
    const convoMessages = await fetchIgConversationMessages({
      conversationId: convo.id,
      accessToken: pageToken,
      version: config.metaApiVersion,
    });
    ...
    for (const msg of convoMessages) {
      messagesProcessed += 1;
      const senderType = msg.from?.id === igId ? 'business' : 'customer';
      if (senderType === 'business') {
        businessCount += 1;
      } else {
        customerCount += 1;
...
export function startInstagramSync(options: {
  db: BetterSQLite3Database;
  config: AppConfig;
  pageId: string;
  igId: string;
